...
// -- Generic Blog Parsing (AWS, Azure, GCP, CNCF) ----------------------------

// Generic blog index parser. Extracts article links by looking for hrefs
// that match the blog's articleBaseUrl pattern.
function parseGenericBlogIndex(html, blog) {
  const articles = [];
  const seenUrls = new Set();
  const baseUrl = blog.articleBaseUrl || blog.indexUrl;
  ...
  while ((match = linkRegex.exec(html)) !== null) {
    ...
    if (href.startsWith('/')) {
      const urlObj = new URL(blog.indexUrl);
      href = `${urlObj.protocol}//${urlObj.host}${href}`;
    }

    // Only include links that look like blog articles (contain the base URL pattern)
    // and aren't the index page itself
    if (!href.includes(baseUrl.replace(/\/$/, '').split('/').pop())) continue;
    if (href === blog.indexUrl || href === blog.indexUrl + '/') continue;
    // Skip anchors, query-only, and non-http
    if (href.startsWith('#') || href.startsWith('?') || !href.startsWith('http')) continue;
    // Skip pagination, tags, categories
    if (/\/(page|tag|category|author|search)\//i.test(href)) continue;

    // Normalize URL (strip trailing slash, query params)
    const cleanUrl = href.split('?')[0].split('#')[0].replace(/\/$/, '');
    if (seenUrls.has(cleanUrl)) continue;
    // Must be longer than the base URL (i.e., has a slug)
    if (cleanUrl.length <= baseUrl.replace(/\/$/, '').length) continue;
...
// Generic article content extractor. Tries JSON-LD, <article> tag,
// <main> tag, then falls back to stripping the whole page.
function extractGenericArticleContent(html) {
  ...
  while ((jsonLdMatch = jsonLdRegex.exec(html)) !== null) {
    try {
      ...
      const item = Array.isArray(ld) ? ld.find(i => i['@type'] === 'BlogPosting' || i['@type'] === 'Article') : ld;
      if (item && (item['@type'] === 'BlogPosting' || item['@type'] === 'Article' || item['@type'] === 'TechArticle')) {
        title = item.headline || item.name || title;
        author = item.author?.name || (Array.isArray(item.author) ? item.author[0]?.name : '') || author;
        publishedAt = item.datePublished || publishedAt;
        break;
      }
    } catch { /* skip */ }
  }
  ...
  if (!publishedAt) {
    const metaDate = html.match(/property="article:published_time"[^>]*content="([^"]+)"/i)
      || html.match(/name="publish[_-]?date"[^>]*content="([^"]+)"/i);
    if (metaDate) publishedAt = metaDate[1];
  }

  // Extract body: try <article>, then <main>, then broad fallback
  const articleMatch = html.match(/<article[^>]*>([\s\S]*?)<\/article>/i);
  const mainMatch = html.match(/<main[^>]*>([\s\S]*?)<\/main>/i);
  const bodyHtml = articleMatch ? articleMatch[1] : mainMatch ? mainMatch[1] : html;
  ...
  // Truncate to ~8000 chars to keep feed file manageable
  ...
  for (const blog of blogs) {
    ...
    try {
      ...
      const indexRes = await fetch(blog.indexUrl, {
        headers: { 'User-Agent': 'FollowCloud/1.0 (feed aggregator)' }
      });
      if (!indexRes.ok) {
      ...
      // Use a generic link-based parser for all cloud blogs
      ...
      for (const article of newArticles) {
        try {
          ...
          // Use generic content extractor for all cloud blogs
          let extracted = extractGenericArticleContent(articleHtml);
